...
Please clone this repo and take a look at [[./branchless_fizzbuzz.html]] instead; or open this file with [[https://www.gnu.org/software/emacs/download.html][GNU Emacs]] if you
don't mind seeing some of the guts. (Blocks marked for noexport, formatting configuration, other stuff that makes the
HTML more legible and this less.)
* Introduction
** Rationale
...
"No conditional logic", huh?

Well, it follows from that, that if we can differentiate numbers based on their prime factors, we can use a lookup table
to determine what to output for that number ('Fizz', 'Buzz', 'FizzBuzz', or the i-to-a representation of the input); and
since for every lookup table, there's an equivalent implementation with logic gates (and vice versa), we can implement
...
- or anything else that mutates ~rip~ in any ways besides advancing it to the next instruction;
- ~mov~ any data from memory to anywhere else;
- perform an indirection of any form other than ~[rdi] + disp64~ (implicit or otherwise; that means ~push~ and ~movs~
  and so forth),
- /or/ mutate ~rdi~ at any point, for any reason.

That means we're gonna need something less pure to do stuff like I/O for us. We'll get to that.
* Implementation
** Overview
...
First, let's define our parameters. As you can see, we need two things from the driver: a buffer to write to, and the
iteration number to process. Both are 64-bit; the former a pointer, the latter simply an unsigned integer. Let's define
...
able to fit the itoa representation ~UINT_64_MAX~ into the right side of the buffer, and ~strlen("FizzBuzz") + 1 = 9~
characters into the left. This allows us to write the strings ~"Fizz\0"~, ~"Buzz\0"~, ~"FizzBuzz\0"~, or ~"000000000"~
...
Let's encode that into our header, and our assembly:
...
With that out of the way, let's define our assembly function. Here's our header:
